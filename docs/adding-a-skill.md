# 新しいスキルを追加する

## 概要

新しいスキルは、共通の雛形から作成する。
仕様を先に確定し、配布物へ反映してから検証する。

共通規則の正本は、[`skill-repository-spec.md`](skill-repository-spec.md) である。
開発環境の構築方法と pytest の実行規約は、[`development-environment.md`](development-environment.md) に従う。

## 1. スキルを定義する

生成前に、スキルの目的と利用例を確定する。
最低限、次の内容を明らかにする。

- スキルが実現する結果
- スキルを使用する依頼や状況
- スキルの対象外とする依頼
- 他のスキルがない場合に単独で目的を満たす方法
- 既存の各スキルと責務が重複するか
- 再利用するスクリプト、参照情報、素材の必要性

利用例から、短く動作を表すスキル名を決める。
スキル名には、小文字の英字、数字、ハイフンだけを使用する。
全スキルのインストールを指定する予約語の `all` は、スキル名に使用しない。

## 2. 雛形を生成する

リポジトリのルートで生成コマンドを実行する。

```bash
python3 scripts/create_skill.py {{skill-name}}
```

生成コマンドは、既存のディレクトリを上書きしない。
生成後に表示されるファイルを順番に編集する。

## 3. 仕様を記載する

最初に `skills/{{skill-name}}/SPEC.md` を編集する。
`todo-` で始まる雛形のプレースホルダーを、すべて具体的な仕様へ置き換える。
たとえば、`{{todo-skill-goal-and-success-criteria}}` をスキルの目的と成功条件へ置き換える。

仕様には、目的、適用条件、独立性と合成、実行時の規則、出力、検証条件を記載する。
重要な規則には、必要に応じて根拠や前提も記載する。

別のスキルを必須条件にしない。
別のスキルを任意に参照する場合は、参照先がなくても同じ要求を満たせるフォールバックを定義する。

## 4. 配布物を実装する

`SPEC.md` の内容を、配布用の `dist/SKILL.md` と `dist/agents/openai.yaml` へ反映する。

`SKILL.md` では、次の点を確認する。

- frontmatter の `name` がスキル名と一致している。
- `description` に機能と適用条件が記載されている。
- 本文に、実行時に従う規則だけが命令形で記載されている。
- 独立性と合成可能性の共通規則が繰り返されていない。
- スキル固有の実行時制約が、対象となる手順または検証の節に記載されている。
- `todo-` で始まるプレースホルダーが残っていない。

`agents/openai.yaml` では、次の点を確認する。

- 表示名と短い説明がスキルの内容を表している。
- `default_prompt` に `${{skill-name}}` が含まれている。
- 暗黙の呼び出しなど、必要な設定だけが追加されている。

実行時リソースが必要な場合だけ、`dist/` の直下へ対応するディレクトリを追加する。
追加できるリソースの種類を次に示す。

- `scripts/`
- `references/`
- `assets/`

## 5. テストを実装する

機械的に結果を判定できる振る舞いがある場合は、`skills/{{skill-name}}/tests/` に pytest のテストを追加する。
実行時スクリプトを追加した場合は、その主要な成功条件と失敗条件を検証する。

対象スキルのテストは、リポジトリのルートから単独で実行できる状態にする。

```bash
python3 -m pytest skills/{{skill-name}}/tests
```

安定して自動判定できる振る舞いがない場合は、空の `tests/` を作成しない。
この場合は、実際の利用者が行う形式の依頼で動作を検証する。

## 6. 概要を記載する

`skills/{{skill-name}}/README.md` に、概要、仕様と配布物へのリンクを記載する。
README に記載しない情報は、次のとおりである。

- 使用方法
- インストール方法
- リポジトリの保守方法

ルート `README.md` の収録スキル一覧にも、リンクと概要を追加する。

スキル固有の適用条件や想定される利用場面が必要な場合は、`SPEC.md` に記載する。
共通の利用方法は、ルート `README.md` だけに記載する。

スキル固有の保守規則がある場合だけ、スキルの直下に `AGENTS.md` を追加する。
共通仕様を `AGENTS.md` へ複製しない。

## 7. 組み合わせ台帳を更新する

ルートの `skill-composition.json` を更新する。
新しいスキルには、単独シナリオを一つ追加する。
新しいスキルと既存の各スキルから成る全 pair を、`orthogonal` または `overlap` に分類する。

`overlap` の pair には、両スキルを含む代表的な組み合わせシナリオを指定する。
同じシナリオで複数の pair を検証できる場合は、シナリオを重複させない。
任意参照がある場合は、参照箇所とフォールバックも記載する。

## 8. 検証する

対象スキルを指定して、構造とメタデータを検証する。

```bash
python3 scripts/validate_skills.py {{skill-name}}
```

雛形の未確定項目や、ルート README への追加漏れも検出される。
単独シナリオ、全 pair の分類、組み合わせシナリオ、任意参照の不整合も検出される。
検査結果を修正し、成功するまで再実行する。

台帳で追加または変更した単独シナリオと組み合わせシナリオを、実際の利用者が行う形式の依頼で前向き評価する。
評価では、シナリオに列挙したスキルだけを導入した新しいセッションを使用する。
期待結果と禁止結果を確認し、結果を最終報告へ記載する。
