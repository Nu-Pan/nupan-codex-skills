...
根拠は、依存する挙動から対象版のソースと実測へたどれる形で残す。依存理由と revision、実行条件が分かり、ソースで確認したこと、実測結果、残る不確実性を区別できる説明にする。

変更の依頼では、既存の設計文書など一箇所へ詳細を集約し、integration code の必要な箇所から参照する。適切な文書がなければ `docs/codex-cli-behavior-evidence.md` を作る。最終報告では要点と検証結果、根拠への参照を示す。参照先がなければ、報告自体に production permalink と必要な根拠を載せる。
