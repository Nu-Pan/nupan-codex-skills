...
数値は、入力が表す十進数の値を保って検査する。例えば、`1` と `1.0` は同値として扱い、整数を要求するキーワードでも、小数・指数表記が整数値を表す場合は受理する。異なる値を読み込み時の丸めによって同値にせず、指数表記の有限値を `Infinity` やゼロへ変換しない。診断へ数値を含める場合もその値を保つ。
...
`$defs` とローカルの `$ref` を許可する。`$ref` は、`#`、または `#/` で始まる [JSON Pointer](https://www.rfc-editor.org/rfc/rfc6901) とし、文書内のスキーマオブジェクトを指すものとする。配列の添字は `0`、または `1`〜`9` で始まる ASCII 数字列とし、範囲外の添字も参照の違反として診断する。

参照先にも同じプロファイルのスキーマ規則を適用し、違反は参照先の位置へ診断する。JSON オブジェクトであることだけではスキーマとして適合したことにならない。再帰参照では、循環を検出して展開を止める。
...
配布 CLI のテストでは、入力の値を保った判定、プロファイル規則、診断形式、終了コードを検証する。例えば、大きな整数も値を保って扱えることと、正規表現のコンパイル失敗を入力違反として診断できることを確認する。また、上限値と超過値の境界、および共有参照と再帰参照を組み合わせた入力で検証が完了することを確認する。診断の順序と必要な情報を確かめ、出力例の文言や JSON オブジェクトのキー順を契約に追加しない。複数の規則が同じスキーマの検証を求める場合は、一回の結果を共用する。
