...
配布 CLI のテストでは、入力の値を保った判定、プロファイル規則、診断形式、終了コードを検証する。また、上限値と超過値の境界、および共有参照と再帰参照を持つ入力で検証が完了することを確認する。診断の順序と必要な情報を確かめ、出力例の文言や JSON オブジェクトのキー順を契約に追加しない。複数の規則が同じスキーマの検証を求める場合は、一回の結果を共用する。
