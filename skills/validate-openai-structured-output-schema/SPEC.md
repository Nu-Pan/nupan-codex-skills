...
対象ごとに配布 CLI を実行し、全診断を確認する。
修正の依頼では、診断箇所と関連する定義を一緒に読み、意図する契約を保って不整合を解消する。
レビューだけの依頼ではファイルを変更しない。
修正後は同じプロファイルで再検証する。

診断はプロファイル適合性の根拠として扱う。
アプリの正本仕様を validator の実装から逆算しない。
一般の JSON Schema validator の成功では、このプロファイルの検証を代用できない。
API や Codex CLI の呼び出しは標準手順に含めない。
