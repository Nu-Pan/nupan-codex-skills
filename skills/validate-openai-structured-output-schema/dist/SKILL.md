...
未知の keyword もエラーになる。そのため、一般の JSON Schema validator の成功では、この固有のプロファイルの検証を代用しない。

## 診断を解消して報告する

修正の依頼では、診断箇所と関連する定義を一緒に読み、意図する契約を保って不整合を解消する。その際、契約の根拠はアプリの正本に、プロファイル適合性の根拠は診断結果に求める。一方、レビューだけの依頼ではファイルを変更しない。

修正後は同じプロファイルで再検証し、同じ schema の検査は併用する規則と共用する。報告は、どのファイルをどのプロファイルで検証し、適合したか、違反が残るか、検証できなかったかが分かるものにする。残る違反は診断コードと JSON Pointer から原因へたどれるようにする。

API や Codex CLI を呼び出さずに検証する。ただし、この成功はプロファイルへの適合を意味し、リモートサービスの受理を保証しない。
